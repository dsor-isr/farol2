...
from farol2_interfaces.msg import ProcessState
from farol2_interfaces.srv import ManageProcess
...
class Process:
  ...
  def start(self):
    if not self.isActive():
      ...
      if 'ros2 launch farol2_bringup ' in self.cmd and not any(arg.startswith('use_sim_time:=') for arg in cmd):
...
class FarolBringup(Node):
  ...
  def loadParams(self):
    ...
    self.declare_parameter('farol_bringup_package_path_share', 'farol2_bringup')
  ...
  def createROSTempConfigs(self):
    ...
    # every process launched from farol2_bringup reads a consistent value.
...
def main(args=None):
  ...
  farol2_bringup = FarolBringup()

  try:
    rclpy.spin(farol2_bringup)
  except KeyboardInterrupt:
    pass
  finally:
    farol2_bringup.cleanRosProcesses()
    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    farol2_bringup.destroy_node()
    rclpy.shutdown()
