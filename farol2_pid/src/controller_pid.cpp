...
void ControllerPID::configure(double kp, double ki, double kd, double kffv_lin, double kffv_sq,
  ...
  configure(kp, ki, kd, kffv_lin, kffv_sq, kffa, tau_min, tau_max, use_ref_lpf, lpf_wc,
...
double ControllerPID::callController(double state, double state_ref, double state_rate, double dt) {
  ...
  if (!first_it_) {
    ...
    state_dot_ = farol2_utils::wrapToPi(state_for_control - state_prev_) / dt;
  ...
  if(delta_implementation_){
    ...
    tau_d_ = -ki_ * error_ - kp_ * error_dot_ - kd_ * state_rate_dot_ + kffv_lin_ * state_rate_dot_ + kffv_sq_ * state_rate_dot_ * abs(state_rate_dot_);

    // for debug only
    p_term_ = -kp_ * error_dot_;
    i_term_ = -ki_ * error_;
    d_term_ = -kd_ * state_rate_dot_ + kffv_lin_ * state_rate_dot_ + kffv_sq_ * state_rate_dot_ * abs(state_rate_dot_);
    ...
    output_ = std::clamp(tau_sat_ + kd_ * dref_ +  kffa_ * ddref_dot_, tau_min_, tau_max_);
  }
