...
from farol2_waypoint.srv import SendWpType1
...
\tconsole addons\tstarts the http server 
...
def cmd_set_topic(args, console_server: Node):
  ...
  for k in topics:
    ...
    if(console_server.get_parameter('topics.addons.waypoint').get_parameter_value().string_value in topic_name):
      try:
...
# so I had to change the message for the addons state
# In order to keep the message parsable on the side of the web addons,
...
class ConsoleServer(Node):
  ...
  def loadParams(self):
    ...
    self.declare_parameter('topics.addons.waypoint', 'wp_standard')
