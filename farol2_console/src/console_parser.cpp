...
void ConsoleParser::requestPath() {
  ...
  std::shared_ptr<farol2_planning::srv::ResetPath::Request> req = std::make_shared<farol2_planning::srv::ResetPath::Request>();
  req->reset_path = true;
  ...
  for(it = mission.begin(); it != mission.end(); ++it) {
    ...
    if(it->type == 2) {
      ...
      if(!(it->xi == it->xe && it->yi == it->ye)) {
        ...
        std::shared_ptr<farol2_planning::srv::SpawnLine::Request> req = std::make_shared<farol2_planning::srv::SpawnLine::Request>();
        ...
        std::shared_ptr<farol2_planning::srv::SetConstSpeed::Request> speed_req = std::make_shared<farol2_planning::srv::SetConstSpeed::Request>();
      ...
      if(!((it->xi == it->xe && it->yi == it->ye) || 
        ...
        std::shared_ptr<farol2_planning::srv::SpawnArc2D::Request> req = std::make_shared<farol2_planning::srv::SpawnArc2D::Request>();
        req->start_point[1] = xrefpoint + it->xi;
        req->start_point[0] = yrefpoint + it->yi;
  ...
  if(run >= 1) {
      ...
      std::shared_ptr<farol2_path_following::srv::StartPF::Request> req = std::make_shared<farol2_path_following::srv::StartPF::Request>();
