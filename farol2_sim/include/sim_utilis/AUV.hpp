...
public:
     ...
     * @param min_max_thruster_input The minimum dead-zone and maximum thruster RPM inputs [min_rpm, max_rpm]^T
     ...
     * @param thrust A vector of n elements with the RPM command to apply to each of the n vehicle thrusters
     */
    ...
    Eigen::Vector3d getOceanCurrent() const;
...
private:
    ...
    Eigen::Vector3d last_ocean_disturbances_ = Eigen::Vector3d::Zero();

    /**
     *
     * @param thrust Method that given the desired RPM input to apply to each thruster computes
     * the actually applied thrust in (N). This model saturates the input between the configured RPM limits, applies a dead-zone,
     ...
     * Then it calls the auxiliary method convertThrustToForce() to convert the output RPM to forces expressed
     * in Newton.
    ...
    /**
     * Takes double with the thruster RPM and converts it to Newtons according to a parabola thrust curve
     * using the equation:
     * force (N) = a*{x}^2 + b*x +c
     ...
     * @param thrust The RPM input applied by one thruster
    ...
    double thrusters_pole_, thrusters_delay_, sampling_period_;
