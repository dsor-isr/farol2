...
Aguiar::Aguiar(double delta, double kk[2], double kz, double k_pos, double k_currents, 
...
bool Aguiar::setPFGains(std::vector<double> gains) {
...
void Aguiar::callPFController(double dt) {
  ...
  path_pos << this->path_state_.pd[0], this->path_state_.pd[1];
  ...
  //TODO: check sway_vel later
  //sway_vel << 0.0, this->vehicle_state_.v1[1];  
  sway_vel << 0.0, 0.0;
  ...
  if(this->first_iteration_) {
    /* If it is the first iteration, just initiate the estimator */
    this->pos_hat_ = vehicle_pos;
    this->first_iteration_ = false;
  }

  Eigen::Vector2d pos_hat_dot = rot_inert * vehicle_v1 + this->currents_hat_ + this->k_pos_ * (vehicle_pos - this->pos_hat_);
  this->pos_hat_ += pos_hat_dot * dt;

  Eigen::Vector2d currents_hat_dot = this->k_currents_ * (vehicle_pos - this->pos_hat_);
  this->currents_hat_ += currents_hat_dot * dt;

  //TODO: fix currents estimation
  //right now it's disabled (wasnt working properly with real vehicles)
  ...
  /* PF Control law */
  Eigen::Vector2d vel_yaw_desired = this->delta_inv_ * (-this->kk_ * pos_error_tanh - sway_vel - this->currents_hat_ + rot_body * pd_dot * (this->path_state_.vd + this->path_state_.vc));
  ...
  pfollowing_debug_.algorithm = "Aguiar";
...
void Aguiar::publish_private(){
  ...
  msg.data = this->desired_yaw_rate_ / 180.0 * M_PI;
  this->yaw_rate_pub_->publish(msg);
...
bool Aguiar::resetVirtualTarget(float value) {
