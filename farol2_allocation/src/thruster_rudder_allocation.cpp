...
void ThrusterRudderAllocation::loadParams() {
  ...
  nr_thrusters_ = declare_parameter<int>("allocation.thrusters.n_thrusters");
  for (size_t i = 0; i < nr_thrusters_; ++i){
    declare_parameter<std::string>("allocation.thrusters.configuration."+ std::to_string(i) + ".name");
    declare_parameter<std::vector<double>>("allocation.thrusters.configuration."+ std::to_string(i) + ".moment_arms");
    declare_parameter<std::vector<double>>("allocation.thrusters.configuration."+ std::to_string(i) + ".angles");
  }

  /* Angular limits for the rudder */
  rudder_angle_min_ = declare_parameter<double>("allocation.rudder.limits.min")/180*M_PI;
  rudder_angle_max_ = declare_parameter<double>("allocation.rudder.limits.max")/180*M_PI;
