...
WpLoose::WpLoose(rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr surge_pub,
...
void WpLoose::calculateRef(Vehicle_t state, WPref_t wp_ref, bool turn_radius_flag) {
    ...
    if (distance_to_wp > cdist) {
      ...
      yaw_ref =
          -atan2(-(x - x_ref), -(y - y_ref)) * 180.0 / M_PI +
          90.0;
      ...
      double gain = tanh(-fabs(yaw_err) * 2 * M_PI / err_0_gain +
                         M_PI) *
                        0.5 +
                    0.5; // normal
