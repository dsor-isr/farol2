...
Arc2D::Arc2D(Eigen::Vector2d start_point, Eigen::Vector2d end_point, Eigen::Vector2d center_point, int direction, double z) : PathSection(true) {
  ...
  double max_gamma = (direction == -1) ? FarolUtils::wrapTo2Pi(this->psif_ - this->psi0_)/M_PI :           // if turning right (-1)
                                         (2*M_PI - FarolUtils::wrapTo2Pi(this->psif_ - this->psi0_))/M_PI; // if turning left  (+1)
...
double Arc2D::getClosestPointGamma(Eigen::Vector3d &coordinate) {
    ...
    double psi_init = FarolUtils::wrapTo2Pi(this->psi0_);
    double psi_end = FarolUtils::wrapTo2Pi(psie);
    double psi_act = FarolUtils::wrapTo2Pi(psi);
      
    psi_end = FarolUtils::wrapTo2Pi(psi_end - psi_init);
    psi_act = FarolUtils::wrapTo2Pi(psi_act - psi_init);
